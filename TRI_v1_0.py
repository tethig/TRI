...
if __name__ == "__main__":
    ...
    output_name = str(input("Please write name of output file [default={}] (.tsv will be appended): ".format(default_name))) or default_name
    ...
    # analyse for repeats
    with open(output_name + '.tsv', 'w') as outfile:
        # header lines X2
        print("#TandemRepeatIdentifier v.{}. Settings: motif_length: {}, min_reps: {}.\ngenome\tmotif\tgenomesize\tstart\tend\tsequence\tstrand".format(
            __version__, motif_length, min_reps), file=outfile)
        # loop through each sequence
        for chrom, seq in sequences.items():
            # loop through each motif
            for unit in motifs:
                regex = re.compile("({}){{{},}}".format(unit, min_reps), re.IGNORECASE)
                discovery = [(len(seq), m.start()+1, m.end(), m.group()) for m in re.finditer(regex, seq)] #is list comprehension
                for tup in discovery:
                    print(chrom, "\t", unit, "\t", sep = "", end = "", file = outfile)
